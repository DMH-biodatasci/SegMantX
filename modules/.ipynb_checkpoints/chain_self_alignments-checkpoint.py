...
def main():
    ...
    parser.add_argument("-i", "--input_file", required=True, type=str, help="Input file received from 'generate_alignments' (required, i.e., tab-delimited five columns: q.start, q.end, s.start, s.end, identity). Alternatively, provide BLAST output format 7 and use --blast_outfmt7 flag).")
    ...
    parser.add_argument("-Q", "--is_query_circular", action="store_true", default=False, help="Indicates if the query sequence has a circular sequence topology (Default: False).")
    parser.add_argument("-L", "--sequence_length", type=int, default=None, help="Size of the sequence (is required with circular sequence topology, or, otherwise, provide fasta file (i.e., using --fasta_file)) (Default: None).")
