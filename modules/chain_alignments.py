...
def main():
    ...
    parser.add_argument("-i", "--input_file", type=str, required = True, help="Input file received from 'generate_alignments' (required, i.e., tab-delimited five columns: q.start, q.end, s.start, s.end, identity). Alternatively, provide BLAST output format 7 and indicate --blast_outfmt7 flag).")
    ...
    parser.add_argument("-Q", "--is_query_circular", action="store_true", help="Indicates if the query sequence has a circular sequence topology (Default: False).")
    parser.add_argument("-S", "--is_subject_circular", action="store_true", help="Indicates if the subject sequence has a circular sequence topology (Default: False).")
